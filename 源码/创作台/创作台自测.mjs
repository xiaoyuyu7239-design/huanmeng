...
// 平台化契约：全部在架作品均携带正式 companion，发布校验 0 错误、创作资产完成度 100%。
...
for (const 条目 of 正式名录.featured) {
  ...
  const companion = JSON.parse(await readFile(new URL(`../../公共资源/games/${条目.slug}/creator.json`, import.meta.url), 'utf8'));
  ...
  assert.deepEqual(报告.errors, [], `${条目.slug} 携带正式 companion 后不得存在发布错误`);
  assert.equal(
    计算创作资产完成度(项目).percentage,
    100,
    `${条目.slug} 创作资产完成度必须为 100%`,
  );
...
// 无 companion 的项目仍必须被三条阻塞规则拦住（能力用合成样例承接，防止旁路发布）。
const 无配套项目 = 由剧情构造项目('no-companion-sample', 第九席剧情, null);
assert.equal(
  运行校验(无配套项目).errors.filter((错误) => 错误.includes('阻塞规则') && 错误.includes('尚未给出人工复核结论')).length,
  3,
  '无 companion 项目必须保留三条 error 级待复核阻塞',
);
